...
import { RenderTargetGlowPostProcess } from './RenderTargetGlowPostProcess'

export class RenderTargetGlowPostPostProcessPrePass {
  ...
  public _maxSize: number = 0
  public _mainTextureDesiredSize: ISize = { width: 0, height: 0 }
  public _mainTexture!: RenderTargetTexture
  public _shouldRender = true
  public _postProcesses: PostProcess[] = []
  public _finalPass!: PostProcess
  public _textures: BaseTexture[] = []
   ...
   */
  public onDisposeObservable = new Observable<RenderTargetGlowPostPostProcessPrePass>()

  /**
   * An event triggered when the effect layer is about rendering the main texture with the glowy parts.
   */
  public onBeforeRenderMainTextureObservable = new Observable<RenderTargetGlowPostPostProcessPrePass>()

  /**
   * An event triggered when the generated texture is being merged in the scene.
   */
  public onBeforeComposeObservable = new Observable<RenderTargetGlowPostPostProcessPrePass>()
   ...
   */
  public onAfterComposeObservable = new Observable<RenderTargetGlowPostPostProcessPrePass>()

  /**
   * An event triggered when the effect layer changes its size.
   */
  public onSizeChangedObservable = new Observable<RenderTargetGlowPostPostProcessPrePass>()
  ...
  constructor(
    ...
    name: string,
    scene: Scene,
    private _renderTargetPostProcess: RenderTargetGlowPostProcess
  ) {
    this.name = name

    this._scene = scene || EngineStore.LastCreatedScene
    RenderTargetGlowPostPostProcessPrePass._SceneComponentInitialization(this._scene)

    this._engine = this._scene.getEngine()
    this._maxSize = this._engine.getCaps().maxTextureSize
    // TODO: where to add?
    // this._scene.effectLayers = this._scene.effectLayers ?? []
    // this._scene.effectLayers.push((this as unknown) as EffectLayer)
  ...
  private _setupFinalPass() {
    ...
    void main(void) {
        vec4 baseColor = texture(texture1, vUV);
        baseColor += texture(texture2, vUV);
        baseColor += texture(texture3, vUV);
        gl_FragColor = baseColor * offset;
    }

`

    this._finalPass = new PostProcess(
      'Final compose shader',
      'final', // shader
      null, // attributes
      ['texture1', 'texture2', 'offset'],
      1.0,
    ...
    this._finalPass.alphaMode = Constants.ALPHA_ADD

    this._finalPass.onApply = effect => {
      effect.setTexture('texture1', this._renderTargetPostProcess._blurTexture1)
      effect.setTexture('texture2', this._renderTargetPostProcess._blurTexture2)
      effect.setTexture('texture3', this._mainTexture)
      effect.setFloat('offset', this._renderTargetPostProcess.intensity)
    }
  }
   ...
   */
  public init(options: Partial<IEffectLayerOptions>): void {
    // Adapt options
    ...
    this._renderTargetPostProcess._createTextureAndPostProcesses()
    this._mergeDrawWrapper.setEffect(this._renderTargetPostProcess._createMergeEffect())
  }
   ...
   */
  public _isReady(subMesh: SubMesh, useInstances: boolean, emissiveTexture: Nullable<BaseTexture>): boolean {
   ...
   */
  public render(applyOnCanvas = false): void {
    ...
    if (applyOnCanvas) {
      ...
      this._renderTargetPostProcess._internalRender(currentEffect.effect!)
  ...
  protected _renderSubMesh(subMesh: SubMesh, enableAlphaMode: boolean = false): void {
    ...
    this._renderTargetPostProcess._setEmissiveTextureAndColor(renderingMesh, subMesh, material)
  ...
  public getClassName(): string {
    return 'RenderTargetEffectLayer'
  }
...
function _WarnImport(arg0: string) {
