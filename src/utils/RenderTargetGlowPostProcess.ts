...
import { RenderTargetGlowPostPostProcessPrePass } from './RenderTargetGlowPostProcessPrePass'
import {
  serialize,
  IGlowLayerOptions,
  BlurPostProcess,
 ...
 */
export class RenderTargetGlowPostProcess {
  /**
  ...
  public _prePass: RenderTargetGlowPostPostProcessPrePass
  ...
  public _blurTexture1!: RenderTargetTexture
  public _blurTexture2!: RenderTargetTexture
   ...
   */
  public customEmissiveColorSelector: Nullable<(mesh: Mesh, subMesh: SubMesh, material: Material, result: Color4) => void> = null
  /**
   * Callback used to let the user override the texture selection on a per mesh basis
   */
  public customEmissiveTextureSelector: Nullable<(mesh: Mesh, subMesh: SubMesh, material: Material) => Texture> = null

  /**
   * Instantiates a new glow Layer and references it to the scene.
   * @param name The name of the layer
   * @param _scene The scene to use the layer in
   * @param options Sets of none mandatory options to use with the layer (see IGlowLayerOptions for more information)
   */
  constructor(name: string, private _scene: Scene, private _engine: Engine, options?: Partial<IGlowLayerOptions>) {
    this._prePass = new RenderTargetGlowPostPostProcessPrePass(name, _scene, this)
    this._prePass.neutralColor = new Color4(0, 0, 0, 1)
    ...
    this._scene.customRenderTargets = [this._prePass._mainTexture]
  }

  public render(applyOnCanvas = false) {
    this._prePass.render(applyOnCanvas)
  }
  ...
  public _createTextureAndPostProcesses(): void {
    var blurTextureWidth = this._prePass._mainTextureDesiredSize.width
    var blurTextureHeight = this._prePass._mainTextureDesiredSize.height
    blurTextureWidth = this._engine.needPOTTextures ? Engine.GetExponentOfTwo(blurTextureWidth, this._prePass._maxSize) : blurTextureWidth
    blurTextureHeight = this._engine.needPOTTextures ? Engine.GetExponentOfTwo(blurTextureHeight, this._prePass._maxSize) : blurTextureHeight
    ...
    this._prePass._textures = [this._blurTexture1, this._blurTexture2]
    ...
    this._horizontalBlurPostprocess1.onApplyObservable.add(effect => {
      effect.setTexture('textureSampler', this._prePass._mainTexture)
    })
    ...
    this._prePass._mainTexture.samples = this._options.mainTextureSamples!
    this._prePass._mainTexture.onAfterUnbindObservable.add(() => {
    ...
    // Prevent autoClear.
    this._prePass._postProcesses.map(pp => {
      pp.autoClear = false
    })
  }
  ...
  public isReady(subMesh: SubMesh, useInstances: boolean): boolean {
    ...
    return this._prePass._isReady(subMesh, useInstances, emissiveTexture)
  }
   ...
   */
  public _internalRender(effect: Effect): void {
    // Texture
    effect.setTexture('textureSampler', this._blurTexture1)
   ...
   */
  public _setEmissiveTextureAndColor(mesh: Mesh, subMesh: SubMesh, material: Material): void {
    var textureLevel = 1.0

    if (this.customEmissiveTextureSelector) {
      this._prePass._emissiveTextureAndColor.texture = this.customEmissiveTextureSelector(mesh, subMesh, material)
    } else {
      if (material) {
        this._prePass._emissiveTextureAndColor.texture = (<any>material).emissiveTexture
        if (this._prePass._emissiveTextureAndColor.texture) {
          textureLevel = this._prePass._emissiveTextureAndColor.texture.level
        }
      } else {
        this._prePass._emissiveTextureAndColor.texture = null
      }
    }

    if (this.customEmissiveColorSelector) {
      this.customEmissiveColorSelector(mesh, subMesh, material, this._prePass._emissiveTextureAndColor.color)
    } else {
      if ((<any>material).emissiveColor) {
        ...
        this._prePass._emissiveTextureAndColor.color.set(
        ...
        this._prePass._emissiveTextureAndColor.color.set(
          this._prePass.neutralColor.r,
          this._prePass.neutralColor.g,
          this._prePass.neutralColor.b,
          this._prePass.neutralColor.a
        )
      }
    }
  }
